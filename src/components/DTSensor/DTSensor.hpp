...
{
public:
  DTComponent(mgos_config_dtcomp *cfg);
...
private:
  mgos_config_dtcomp *_cfg{nullptr};
...
DTComponent::DTComponent(mgos_config_dtcomp *cfg)
    : PollingComponent("DTComponent", 10000), _cfg(cfg)
...
Status DTComponent::Init()
{
  ...
  _controller = new DallasRmt(_cfg->port, 0, 1);
...
// save sensors to file system
Status DTComponent::saveSensors()
{

mgos_sys_config_set_dtcomp_saved((int)true);
return Status::OK();
}

Status DTComponent::loadSensors()
{
return Status::OK();
}
